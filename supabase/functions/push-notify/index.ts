...
serve(async (req) => {
  ...
  try {
    ...
    // ── ACTION: admin_notify_user (إشعار موجّه من الأدمن لمستخدم واحد) ─────────
    if (body.action === 'admin_notify_user') {
      const authHeader = req.headers.get('Authorization');
      const token = authHeader?.replace('Bearer ', '');
      if (!token) {
        return new Response(
      ...
      const { target_user_id, title, message } = body as { target_user_id: string; title: string; message: string };
      if (!target_user_id || !title?.trim() || !message?.trim()) {
        return new Response(
          JSON.stringify({ error: 'target_user_id, title and message are required' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
        );
      }

      const { data: targetProfile } = await supabaseAdmin.from('user_profiles').select('push_token').eq('id', target_user_id).single();
      const pushToken: string | null = targetProfile?.push_token ?? null;
